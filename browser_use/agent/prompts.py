...
class SystemPrompt:
	...
	def _load_prompt_template(self) -> None:
		...
		template_filename = (
			...
			else 'system_prompt_no_thinking.md'
		)

		# 1) Preferred: look up the file via importlib.resources (works for
		#    regular packages).
		try:
			with (
				importlib.resources.files(__package__)  # 'browser_use.agent'
			...
			# Namespace/editable installs may return a MultiplexedPath that
			# cannot open files.  Fall back to path relative to this file.
			pass

		# 2) Fallback: direct path next to this .py file – always available in
		#    a source checkout.
