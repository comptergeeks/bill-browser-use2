...
class ChatGoogle(BaseChatModel):
	...
	def get_client(self) -> Any:
		"""
		Returns the configured google.generativeai module (acts like a client).

		Returns:
			Any: The generativeai module after configuration.
		"""
		client_params = self._get_client_params()

		# The google.generativeai library is usually configured via genai.configure.
		# We mimic a lightweight "client" by returning the module itself after
		# applying configuration parameters that exist.
		configure_fn = getattr(genai, 'configure', None)
		if callable(configure_fn) and 'api_key' in client_params:
			configure_fn(api_key=client_params['api_key'])

		return genai  # type: ignore[return-value]
		...
		if response.usage_metadata is not None:
			...
			if response.usage_metadata.prompt_tokens_details is not None:
				image_tokens = sum(
					...
					if getattr(detail, 'modality', '').upper() == 'IMAGE'
				)
